...
function set_secrets {
    ...
    readarray -d '' files_affected < <(find "$TARGET_PATH/sql" -type f -print0)

    for file in "${files_affected[@]}"; do
        regex="(user[\\s]*=).+"
        replace_by_regex $regex $mysql_user {}

        regex="(password[\\s]*=).+"
        replace_by_regex $regex $mysql_pwd {}

        regex="(dbname[\\s]*=).+"
        replace_by_regex $regex $mysql_db {}
    done
